/******/ (() => { // webpackBootstrap
  ...
  locationBtn.addEventListener('click', () => {
    ...
    // Fetch weather data from weather API using latitude and longitude
    async function getLocationFromCoords(lat, lon) {
        try{
          const response = await fetch(`https://api.weatherapi.com/v1/current.json&forecast.json?key=cd77a577f4344b949c0205340231505&q=${lat},${lon}&days=3`, {mode: 'cors'});
          ...
          return locationInfo;
            ...
            mainContent.style.display = 'none';
            errorMsg.textContent = "The request to the API failed";
        }
      }
    ...
    // Verify if browser accepts geo locate
    function geoFind() {
    ...
    function success(location) {
        ...
        (async () => {
            getWeatherDataGeoLocated(latitude,longitude);
        })();
    }
    ...
    // Return weather data object for geo located location
    function getWeatherDataGeoLocated(latitude,longitude) {
        console.log("in the getWeatherDataGeoLocated()");
